...
本文保留 P3 阶段的提取契约与实现记录，其中 L1/L2/L3 规划和证据规则继续由共享产品层使用。下文 DSH workflow 和 DSH 浏览器验收属于旧宿主；当前独立应用通过 `src/standalone/model.ts` 调用模型，运行架构见 [架构说明](architecture.md)。输入格式的后续扩展以当前实现为准。
