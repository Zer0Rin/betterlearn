...
- 两种生成工具共用 UUID 命名空间与持久日志；跨工具复用同一 UUID 会冲突。旧生成请求摘要保持兼容，来源生成摘要增加操作标识。服务在发起生成前持久化请求摘要。同编号同参数返回已记录任务，不重复生成；同编号不同参数报冲突。
...
`test/standalone-mcp.test.ts` 使用真实 Python 服务、假模型供应商和官方 MCP SDK stdio 客户端，验证服务状态、缺失配置失败、出题、重复请求去重、同编号参数冲突、结果不确定时禁止重发、历史共享、密钥不出现在结果、来源认证、退出清理及重启后重连。独立服务与桌面仍共用原有数据锁。

学习 MCP 扩展还验证真实 stdio 学习书/课程发现、来源生成、交卷后的统计/明细、重启重放及只读调用不增加模型请求。真实 MCP 的评估结果与 HTTP 一致，重启后不变；定向测试覆盖只读无需模型、null/0 区分、额外参数拒绝、无请求日志、跨工具 UUID 冲突、来源准备失败无 POST、响应不确定时重启仍禁止重发。生成的 Codex／Claude 配置均通过官方 SDK 的 17 工具验证；本轮未修改前端实现。
