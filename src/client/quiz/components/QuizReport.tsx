...
export function QuizReport({ quiz, records, report, busy, error, onRetry, onRestart }: QuizReportProps) {
  ...
  return <div className="zl-module"><header className="zl-heading"><div className="zl-eyebrow">练习复盘</div><h1>正确率 {accuracy}%</h1><p>{quiz.title}</p></header>
    <section className="zl-report-score zl-panel"><div className="zl-score-ring" style={{ background: `conic-gradient(var(--zl-accent) ${accuracy}%, var(--zl-line) 0)` }}><div><b>{accuracy}<small>%</small></b><span>答题正确率</span></div></div><div><span className="zl-eyebrow">本次练习完成</span><h2>答对 {correct} / {quiz.questions.length} 道题</h2><p>{report?.share_quote || '查看本次答题结果与知识点复盘。'}</p>{records.length > 0 && <div className="zl-report-tags"><span className="zl-tag"><CheckCircle2 size={14}/>答对 {correct} 题</span><span className="zl-tag danger"><XCircle size={14}/>答错 {wrong} 题</span><span className="zl-xp-badge">+{xpGain} XP</span></div>}<span className="zl-tag"><CheckCircle2 size={14}/>已完成逐题回顾</span></div></section>
    {busy && <p className="zl-notice" role="status">正在生成复盘报告，请稍候…</p>}
    {error && <p className="zl-error" role="alert">{error}</p>}
    {!report && !busy && <div className="zl-empty"><p>答题结果已保存在当前标签页，复盘报告尚未生成。</p><button className="zl-primary" onClick={onRetry}>生成复盘报告</button></div>}
