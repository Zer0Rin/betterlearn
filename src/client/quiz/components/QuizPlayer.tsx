...
export function QuizPlayer({ quiz, records, index, onAnswer, onIndexChange, onFinish }: QuizPlayerProps) {
  ...
  return <div className="zl-module">
    ...
    <div className="zl-quiz-layout"><section className="zl-panel zl-question">
      ...
      <div className="zl-question-actions"><button className="zl-secondary" disabled={index === 0} onClick={() => onIndexChange(index - 1)}>上一题</button>{record ? <button className="zl-primary" onClick={() => index === quiz.questions.length - 1 ? onFinish() : onIndexChange(index + 1)}>{index === quiz.questions.length - 1 ? '查看复盘报告' : '下一题'}<ArrowRight size={17}/></button> : <button className="zl-primary" disabled={!selected.length} onClick={() => onAnswer({ question_id: question.id, selected_answers: selected, is_correct: isAnswerCorrect(selected, question.answer), duration_ms: Math.max(0, Date.now() - started) })}>确认答案</button>}</div>
    </section><aside className="zl-panel zl-question-map"><h3>本次练习</h3><div className="zl-question-dots">{quiz.questions.map((q,i) => { const answer = records.find(r => r.question_id === q.id); return <button key={q.id} aria-label={`第 ${i+1} 题`} aria-current={i === index ? 'step' : undefined} disabled={i > records.length} className={answer ? answer.is_correct ? 'correct' : 'wrong' : ''} onClick={() => onIndexChange(i)}>{i+1}</button> })}</div><p>先独立思考，再查看解析。做错的题，也是在发现新的学习机会。</p></aside></div>
  </div>
}
