...
function answerFacts(records:AttemptAnswers['answer_records']) {
 return JSON.stringify(records.map(r=>({question_id:r.question_id,selected_answers:[...r.selected_answers].sort(),duration_ms:r.duration_ms}))
...
export function ExamAttempt({session,storage,busy,onSave,onSubmit,onReport}:{session:ExamSession;storage:PracticeStorage;busy:boolean;onSave(input:AttemptAnswers):void;onSubmit(input:AttemptAnswers):void;onReport(quizId:string,attemptId:string):void}) {
 ...
 const dirty=session.status!=='submitted'&&answerFacts(input.answer_records)!==answerFacts(session.answer_records)
 ...
 function choose(question:ExamSession['questions'][number],key:string){
  ...
  const next={expected_revision:session.revision,answer_records:[...input.answer_records.filter(r=>r.question_id!==question.id),{question_id:question.id,selected_answers:answers,duration_ms:input.answer_records.find(r=>r.question_id===question.id)?.duration_ms??0}]}
 ...
 return <section className="zl-panel zl-exam-panel"><h2>{session.title}</h2><div className="zl-exam-clock" aria-label="考试倒计时">{expired?'已到截止时间':remaining===0?'本机倒计时已归零，等待服务端核对':`剩余 ${Math.floor(remaining/60)} 分 ${remaining%60} 秒`}</div><p>截止：{goalTime(session.deadline_at)}（{goalTimeZone()}）</p><p className="zl-muted">倒计时按本机时钟估算，以服务端截止为准。退出或刷新不会延长时限；到期只结算此前已保存到服务端的草稿。</p>
  <p role="status">{dirty?'有本地修改尚未保存到服务端':'当前答案与服务端草稿一致'} · 服务端草稿已作答 {session.answer_records.filter(r=>r.selected_answers.length).length} / {session.questions.length} 道</p>
