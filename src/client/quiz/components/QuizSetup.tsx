...
export function QuizSetup({ documents, initialDocId, busy, onGenerate }: Props) {
  ...
  const examples = ['RAG 和传统搜索有什么区别？', '从零理解 Python 装饰器', '学习摄影中的光圈、快门与 ISO']
  return <form className="zl-module zl-panel zl-setup" onSubmit={event => {
    ...
    <div className="zl-setup-body">
      ...
      <label className="zl-input-label" htmlFor="learning-topic">{mode === 'topic' ? '把你想学的，放在这里。' : '想重点练习什么？'}</label>
      <textarea id="learning-topic" placeholder={mode === 'topic' ? '一个好奇的问题、一段笔记，或一个网址…\nAI 会把它变成一组适合你的练习题。' : '可选：指定章节、知识点或学习目标，留空则根据整篇文档出题。'} value={text} onChange={e => setText(e.target.value)} maxLength={2000} rows={5} required={mode === 'topic'} />
      <div className="zl-input-foot"><span>{mode === 'topic' ? '支持主题、文本和网址' : '基于文档检索相关知识'}</span><span>{text.length} / 2000</span></div>
      {mode === 'topic' && <div className="zl-examples"><span>试试</span>{examples.map(example => <button type="button" key={example} onClick={() => setText(example)}>{example}<ArrowUpRight size={12} /></button>)}</div>}
      <div className="zl-settings"><SelectField label="题目数量" value={String(count)} onChange={value => setCount(Number(value))} options={[3,4,5,6,7,8,9,10].map(n => ({ value: String(n), label: `${n} 道题` }))} /><SelectField label="练习难度" value={difficulty} onChange={value => setDifficulty(value as GenerateInput['difficulty'])} options={[{ value: 'mixed', label: '混合难度' }, { value: 'easy', label: '入门' }, { value: 'medium', label: '进阶' }, { value: 'hard', label: '挑战' }]} /><label className="zl-check"><input type="checkbox" checked={images} onChange={e => setImages(e.target.checked)} /><span>AI 题目配图<small>用图像辅助理解</small></span></label></div>
      <div className="zl-start-row"><span><Sparkles size={14} />生成后，逐题练习与讲解</span><button className="zl-primary" disabled={!canSubmit || busy}>{busy ? '正在创建…' : '生成练习'}<ArrowUpRight size={17} /></button></div>
    </div>
  </form>
}
