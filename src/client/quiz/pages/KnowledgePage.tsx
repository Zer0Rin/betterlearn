...
export function KnowledgePage({ api, onPractice }: { api: QuizApi; onPractice(docId: string): void }) {
  ...
  async function upload(file?: File) {
    ...
    setUploading(true)
    try { await api.uploadDocument(file); setNotice('文档已上传，正在解析和建立知识索引。'); setRevision(n => n + 1) }
  ...
  return <><header className="zl-heading"><div className="zl-eyebrow">我的知识库</div><h1>让你的资料，成为练习。</h1><p>课件、工作笔记、阅读材料。上传后，围绕你真正需要的知识出题。</p></header>
    ...
    {loading ? <p className="zl-empty">正在读取文档…</p> : documents.length === 0 ? <div className="zl-panel zl-empty"><FileText size={32}/><h3>第一份资料，从这里开始。</h3><p>上传文档后，可以查看处理进度并开始专属练习。</p></div> : <div className="zl-document-list">{documents.map(doc => <article className="zl-panel zl-document" key={doc.doc_id}><div className="zl-file-icon"><FileText size={23}/><small>{doc.file_type.toUpperCase()}</small></div><div className="zl-document-info"><h3>{doc.file_name}</h3><p><span className={`zl-tag ${doc.status === 'failed' ? 'danger' : ''}`}>{({ processing:'解析中', ready:'已就绪', failed:'解析失败' })[doc.status]}</span><span>{(doc.file_size/1024).toFixed(1)} KB</span>{doc.status === 'ready' && <span>{doc.chunk_count} 个知识片段</span>}</p>{doc.error_message && <p className="zl-inline-error">{doc.error_message}</p>}</div><div className="zl-document-actions"><button className="zl-secondary" disabled={doc.status !== 'ready' || !!deleting} onClick={() => onPractice(doc.doc_id)}>开始练习<ArrowUpRight size={15}/></button><button className="zl-icon-button" aria-label={`删除 ${doc.file_name}`} disabled={!!deleting} onClick={() => void remove(doc)}><Trash2 size={16}/></button></div></article>)}</div>}
    <p className="zl-footnote">文档处理使用已配置的向量模型。若处理失败，请查看提示并检查后端模型配置。</p>
  </>
}
