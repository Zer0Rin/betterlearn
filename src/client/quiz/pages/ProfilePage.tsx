...
export function ProfilePage({ api, onSaved }: { api: QuizApi; onSaved(): void }) {
  ...
  return <><header className="zl-heading"><div className="zl-eyebrow">个人中心</div><h1>你好，{profile?.nickname || '学习者'}。</h1><p>为自己积累知识，也留下看得见的成长。</p></header>{error && <div className="zl-error" role="alert">{error}<button onClick={() => setRevision(n => n+1)}>重新加载</button></div>}
