...
export function Home({ api, docId, onGenerate, busy, profile }: { api: QuizApi; docId?: string; onGenerate(input: GenerateInput): void; busy: boolean; profile?: UserProfile }) {
  ...
  return <><header className="zl-heading"><div className="zl-eyebrow"><span className="zl-small-line"/>把好奇心，变成知识</div><h1>今天，想弄懂什么<span>？</span></h1><p>从一个问题开始。在练习中理解，在复盘中掌握。</p></header>
    <div className="zl-home-grid"><div><QuizSetup key={docId ?? 'topic'} documents={documents} initialDocId={docId} busy={busy} onGenerate={onGenerate}/>{error && <p className="zl-error" role="alert">资料加载失败：{error}<button onClick={()=>setRefresh(n=>n+1)}>重试</button></p>}
      <div className="zl-section-title zl-recent-title"><h2>最近的学习足迹</h2><LocalLink href="#/history">全部记录<ArrowRight size={14}/></LocalLink></div>
      {history.length ? <div className="zl-recent-list">{history.map(item => <LocalLink href={`#/history/${encodeURIComponent(item.quiz_id)}`} key={item.quiz_id} className="zl-recent-item"><span className="zl-recent-icon"><BookOpen size={18}/></span><span><strong>{item.title}</strong><small>{item.question_count} 道题 · 正确率 {Math.round(item.accuracy)}%</small></span><ChevronRight size={16}/></LocalLink>)}</div> : <div className="zl-panel zl-empty-history"><BookOpen size={22}/><div><strong>你的学习足迹，从第一组练习开始</strong><p>完成后的报告会自动收录在这里。</p></div></div>}
    </div><aside className="zl-home-aside"><section className="zl-learning-note"><div className="zl-note-top"><span>LEARN BY DOING</span><Sparkles size={18}/></div><h2>不只是看过，<br/>而是真的懂了。</h2><div className="zl-learning-path"><div><span>01</span><p><b>从好奇出发</b><small>输入主题，或选择自己的资料</small></p></div><div><span>02</span><p><b>在练习中理解</b><small>逐题作答，即时查看讲解</small></p></div><div><span>03</span><p><b>让知识留下来</b><small>回看复盘，找到下一步重点</small></p></div></div><div className="zl-note-bottom"><Target size={15}/>每一次思考，都算数。</div></section>
      <section className="zl-panel zl-week-card"><div><ChartNoAxesCombined size={18}/><span>我的学习积累</span></div><p><b>{profile?.quiz_count ?? 0}</b><span>次练习</span><b>{profile?.total_xp ?? 0}</b><span>经验值</span></p></section>
      <LocalLink href="#/knowledge" className="zl-knowledge-link"><Layers3 size={22}/><div><strong>让资料变成专属练习</strong><small>打开我的知识库</small></div><ArrowRight size={17}/></LocalLink>
    </aside></div>
  </>
}
...
export function QuizWorkspace({ api, storage, onExit, onOpenExtraction, initialRoute = '/' }: { api: QuizApi; storage: Pick<Storage, 'getItem' | 'setItem'>; onExit(): void; onOpenExtraction?(): void; initialRoute?: string }) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        setTaskStatus(result.status === 'running' ? 'AI 正在阅读资料并生成练习…' : '任务已创建，正在准备…')
  ...
  else if ((view === '/quiz' || view === '/report') && session.taskId) content = <div className="zl-panel zl-generation"><div className="zl-generating-orbit"><Sparkles size={35}/></div><div className="zl-eyebrow">一组新的知识，正在成形</div><h1>{taskError ? '还差一点，继续试试。' : taskStatus}</h1><p>检索资料、组织题目、编写讲解。稍等片刻，就可以开始练习。</p>{taskError ? <><p className="zl-error" role="alert">{taskError}</p><button className="zl-primary" onClick={()=>setPollRevision(n=>n+1)}>继续查询</button><button className="zl-secondary" onClick={()=> { setSession(emptySession()); navigate('/') }}>返回重新出题</button></> : <p className="zl-generating-status"><LoaderCircle size={18} className="zl-spin"/>正在处理 · 刷新页面后可继续等待</p>}</div>
