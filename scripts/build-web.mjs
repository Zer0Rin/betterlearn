...
await build({entryPoints:['src/mcp/cli.ts'],outfile:root+'/mcp.mjs',bundle:true,platform:'node',format:'esm',target:'node24',banner:{js:"import { createRequire as __createRequire } from 'node:module'; const require = __createRequire(import.meta.url);"}})
...
for(const path of ['contracts','python/nobei_core','python/requirements-phase1.lock','python/requirements-phase1.txt','services/quiz/app','services/quiz/run_managed.py','services/quiz/requirements.txt','services/quiz/LICENSE','services/quiz/PROVENANCE.md','services/quiz/third_party','LICENSE'])await cp(path,root+'/'+path,{recursive:true,filter:p=>!p.includes('__pycache__')&&!p.endsWith('.pyc')})
