...
def test_load_text_returns_contiguous_content(tmp_path):
    content = "第一段内容。\n\n第二段内容。"
    file_path = tmp_path / "sample.txt"
    file_path.write_text(content, encoding="utf-8")

    text = document_loader_service.load_text(str(file_path), "txt")

    # 必须与原文逐字一致，偏移量才能被外部工具定位
    assert text == content


def test_load_text_joins_pdf_pages(tmp_path):
    file_path = tmp_path / "sample.pdf"
    file_path.write_bytes(b"%PDF-1.4 fake content")

    fake_docs = [
        Document(page_content="第一页" , metadata={"page": 0}),
        Document(page_content="第二页", metadata={"page": 1}),
    ]
    mock_loader_instance = MagicMock()
    mock_loader_instance.load.return_value = fake_docs
    ...
    with patch(
        ...
        text = document_loader_service.load_text(str(file_path), "pdf")

    assert text == "第一页第二页"


def test_load_text_unsupported_format_raises(tmp_path):
    file_path = tmp_path / "sample.xlsx"
    file_path.write_bytes(b"fake content")

    with pytest.raises(ValueError, match="不支持的文件格式"):
        document_loader_service.load_text(str(file_path), "xlsx")
