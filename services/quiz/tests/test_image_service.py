...
@pytest.mark.asyncio
async def test_all_image_failures_return_an_actionable_notice():
    with patch('app.services.image_service.image_repository.get_today_usage_count', AsyncMock(return_value=0)), patch('app.services.image_service.generate_image_for_question', AsyncMock(return_value=None)):
        image_map, notice = await image_service.generate_images_for_quiz([_make_question('q1')], 1, 'quiz_notice')
    assert image_map == {}
    assert notice and '配图' in notice and 'quiz.env' in notice
