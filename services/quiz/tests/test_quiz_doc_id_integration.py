...
@pytest.mark.asyncio
async def test_failed_rag_stops_async_quiz_before_model_call():
    ...
    with (
        patch('app.services.quiz_service.rag_service.fetch_rag_context', new_callable=AsyncMock, side_effect=QuizGenerationError('知识库检索失败')),
        ...
        await _run_quiz_task('test-task', QuizGenerateRequest(user_input='资料重点', doc_id='doc_1'), 1)
        generate.assert_not_awaited()
        assert update.await_args.args == ('test-task', 'failed')
        assert '知识库检索失败' in update.await_args.kwargs['error_message']
