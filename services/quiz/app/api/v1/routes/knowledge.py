...
@router.get("/documents/{doc_id}/content", response_model=ApiResponse)
async def get_document_content(doc_id: str, user_id: int = Depends(get_current_user)):
    result = await knowledge_service.get_document_content(user_id, doc_id)
    return ApiResponse.success(data=result.model_dump())
