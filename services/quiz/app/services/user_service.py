...
LOCAL_WEB_OPENID = "local:web-single-user"
...
async def handle_local_login() -> LoginResponse:
    """为受信任的本机 Web 开发请求签发固定单用户身份。"""
    user = await user_repository.get_or_create_user(LOCAL_WEB_OPENID)
    token = create_token(user_id=user["id"], openid=LOCAL_WEB_OPENID)
    return LoginResponse(
        token=token,
        user=UserBrief(
            id=user["id"],
